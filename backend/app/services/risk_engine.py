...
class RiskEngine:
    ...
    async def extract_features(self, partner_id: str, h3_cell: str) -> dict:
        ...
        cursor = await db["grid_events"].aggregate(pipeline_workability)
        ...
        cursor = await db["partner_activity_logs"].aggregate(pipeline_online)
        result = await cursor.to_list(length=1)
        ...
        cursor = await db["grid_events"].aggregate(pipeline_city_risk)
        result = await cursor.to_list(length=1)
